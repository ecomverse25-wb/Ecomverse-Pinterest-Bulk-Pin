...
const LicenseModal: React.FC<LicenseModalProps> = ({ onActivate, onOpenAdmin }) => {
    ...
    const handleActivate = async () => {
        ...
        // --- CHECK ADMIN-MANAGED KEYS IN LOCALSTORAGE ---
        try {
            const storedKeys = localStorage.getItem('ecomverse_license_keys');
            if (storedKeys) {
                const keys = JSON.parse(storedKeys);
                const foundKey = keys.find((k: any) => k.key === cleanedKey && k.active);
                if (foundKey) {
                    setTimeout(() => {
                        localStorage.setItem('ecomverse_license_active', 'true');
                        localStorage.setItem('ecomverse_license_key', cleanedKey);
            ...
            console.error('Failed to check local keys', e);
        }
        // ------------------------------------------------
