...
const pollReplicate = async (getUrl: string, apiKey: string): Promise<string> => {
    while (true) {
        ...
        // Wait 1.5 second before next poll
...
const generateReplicateImage = async (prompt: string, config: PinConfig, apiKey: string): Promise<string> => {
    ...
    switch (config.model) {
        ...
        case 'flux-dev':
            ...
            break;
        case 'sdxl-turbo':
            // stability-ai/sdxl-turbo (Fastest)
            // Note: SDXL Turbo on Replicate often takes explicit width/height
            modelVersion = "stability-ai/sdxl-turbo";
            // Calculate dimensions based on aspect ratio (Approximate for SDXL optimal range)
    ...
    // NOTE: Direct browser calls to Replicate often fail due to CORS. 
    // This code assumes a proxy is setup or the environment supports it.
    const response = await fetch("https://api.replicate.com/v1/models/" + modelVersion + "/predictions", {
        method: "POST",
        headers: {
            'Authorization': `Token ${apiKey}`,
            'Content-Type': 'application/json',
            // 'Prefer': 'wait' // Optional: wait for sync response if supported
        },
...
/**
 * Applies overlays (Logo and CTA) to a base image using HTML Canvas.
    ...
    return new Promise((resolve, reject) => {
        ...
        baseImg.onload = () => {
            ...
            if (config.ctaText) {
                ...
                // Adjust y slightly for visual center of font
            ...
            if (config.logoData) {
                ...
                logoImg.onerror = (e) => {
                    ...
                    // If logo fails, resolve with just base + cta (if drawn)
                    resolve(canvas.toDataURL('image/png'));
                };
